...
export default function ProductCard({ product, index = 0 }: ProductCardProps) {
  ...
  const { addToCart, addToWishlist, isInWishlist } = store;
  const { isAuthenticated } = useAuthStore();
  const navigate = useNavigate();
  ...
  const handleWishlistToggle = async () => {
    ...
    if (inWishlist && typeof (store as any).removeFromWishlist === 'function') {
      await (store as any).removeFromWishlist(product.id);
      return;
    }
    ...
    >
      ...
      <div className="p-4">
        <div className="flex items-center justify-between mb-1">
          ...
          <div className="flex items-center gap-1">
            <FiStar className="w-3 h-3 text-[#C9A86A] fill-[#C9A86A]" />
            <span className="text-[11px] font-body text-[#666]">
        ...
        <div className="flex items-center justify-between mb-2.5">
          ...
          <span
            className={`text-[10px] font-body font-medium ${
              inStock ? 'text-green-600' : 'text-[#C58C85]'
            }`}
          >
            {inStock ? 'In Stock' : 'Out of Stock'}
          </span>
        ...
        <div className="flex gap-1.5">
          ...
          {product.colors.length > 4 && (
            <span className="text-[10px] font-body text-[#999] self-center">
              +{product.colors.length - 4}
            </span>
          )}
        </div>
