...
import SectionHeader from '../components/SectionHeader';
import ProductSkeleton from '../components/ProductSkeleton';
import { productsApi, type AdaptedProduct } from '../api/products';

// const heroImage = 'https://images.unsplash.com/photo-1583391733956-6c78276477e1?w=900&q=85';
const sareeImage = 'https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=800&q=80';
const lehengaImage = 'https://images.unsplash.com/photo-1583391733956-6c78276477e1?w=800&q=80';
const kurtiImage = 'https://images.unsplash.com/photo-1622560480605-d83c853bc5c3?w=800&q=80';
const fallbackImage = sareeImage;
...
const testimonials = [
  {
    ...
    avatar: '',
    review:
      'Absolutely in love with my Banarasi saree from Vastrika! The quality is exceptional and it arrived so beautifully packaged. Will definitely shop again.',
    rating: 5,
    product: 'Banarasi Silk Saree',
  },
  {
    ...
    avatar: '',
    review:
      'The Kanjivaram silk saree is stunning! Exactly as pictured and the fabric quality is premium. Vastrika truly delivers luxury at its finest.',
    rating: 5,
    product: 'Kanjivaram Silk Saree',
  },
  {
    ...
    avatar: '',
    review:
      "Ordered the bridal lehenga for my cousin's wedding and I received so many compliments! The embroidery is so intricate. Thank you Vastrika!",
    rating: 5,
    product: 'Bridal Lehenga Choli',
  },
  {
    ...
    avatar: '',
    review:
      'The Chikankari kurti collection is to die for! Each piece is so delicately crafted. Free shipping was a bonus and delivery was super fast.',
    rating: 5,
    product: 'Lucknowi Chikankari Kurti',
  },
];
...
const features = [
  {
    ...
    title: 'Free Shipping Across India',
    description:
      'Complimentary delivery on all orders above ₹999. Fast & secure shipping to your doorstep.',
  },
  {
    icon: FiAward,
    title: 'Premium Fabric Quality',
    description:
      "Every piece is crafted from the finest fabrics sourced directly from India's best weavers.",
  },
  {
    icon: FiRefreshCw,
    title: 'Easy 7-Day Returns',
...
type HomeProducts = {
  trendingProducts: AdaptedProduct[];
  newArrivals: AdaptedProduct[];
  featuredSarees: AdaptedProduct[];
};

let homeProductsCache: HomeProducts | null = null;
let homeProductsRequest: Promise<HomeProducts> | null = null;
...
function setFallbackImage(event: SyntheticEvent<HTMLImageElement>, fallback = fallbackImage) {
...
function avatarFor(name: string, avatar?: string) {
  ...
  return `https://ui-avatars.com/api/?name=${encodeURIComponent(name)}&background=EFE7DC&color=7A4E48`;
}

export default function Home() {
  const [trendingProducts, setTrendingProducts] = useState<AdaptedProduct[]>(
    homeProductsCache?.trendingProducts ?? []
  );
  const [newArrivals, setNewArrivals] = useState<AdaptedProduct[]>(
    homeProductsCache?.newArrivals ?? []
  );
  const [featuredSarees, setFeaturedSarees] = useState<AdaptedProduct[]>(
    homeProductsCache?.featuredSarees ?? []
  );
  const [loadingTrending, setLoadingTrending] = useState(!homeProductsCache);
  const [loadingNewArrivals, setLoadingNewArrivals] = useState(!homeProductsCache);
  const [loadingFeaturedSarees, setLoadingFeaturedSarees] = useState(!homeProductsCache);
  ...
  return (
    <div className="pt-[72px]">
          ...
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center min-h-[calc(100vh-72px)]">
            ...
            >
              ...
              <motion.h1
                ...
                className="font-heading text-5xl md:text-6xl lg:text-7xl font-semibold text-[#1F1F1F] leading-tight mb-6"
              >
                Graceful Fashion for <span className="text-[#7A4E48] italic">Modern</span>{' '}
                Indian Women
              </motion.h1>
              ...
              <motion.p
                ...
                className="font-body text-base md:text-lg text-[#666] leading-relaxed mb-8 max-w-md"
              >
                Premium handcrafted ethnic wear designed with timeless elegance and comfort. From
                Banarasi silks to Chikankari kurtis, every piece tells a story.
              </motion.p>
              ...
              >
                {[
                  ...
                  <div key={stat.label}>
                    <div className="font-heading text-2xl font-bold text-[#7A4E48]">
                      {stat.num}
                    </div>
                    <div className="font-body text-xs text-[#999]">{stat.label}</div>
                  </div>
              ...
              <div className="relative">
                ...
                <div className="relative rounded-[2rem] overflow-hidden aspect-[4/5] shadow-2xl">
                  <img
                    src="hero-saree.jpg"
                    alt="Indian Ethnic "
                    onError={(event) => setFallbackImage(event, lehengaImage)}
                    className="w-full h-full object-cover"
                  />
                  <div className="absolute inset-0 bg-gradient-to-t from-[#1F1F1F]/30 via-transparent to-transparent" />
                </div>
                ...
                >
                  <p className="font-body text-xs text-[#999] mb-1">Today's Pick</p>
                  <p className="font-heading text-sm font-semibold text-[#1F1F1F]">
                    Kanjivaram Silk
                  </p>
                  <p className="font-heading text-base font-bold text-[#7A4E48]">₹14,999</p>
                </motion.div>
      ...
      <div className="bg-[#7A4E48] py-3 overflow-hidden">
        <div className="flex gap-0 whitespace-nowrap">
          <div className="flex gap-8 animate-marquee font-body text-xs text-white/80 font-medium uppercase tracking-[0.15em]">
            {Array(3)
              .fill([
                'Free Shipping Above ₹999',
                'Handcrafted by Indian Artisans',
              ...
              .flat()
              .map((text, i) => (
                <span key={`${text}-${i}`} className="flex-shrink-0">
                  {text}
                </span>
              ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {promobanners.map((banner, i) => (
              ...
              >
                <Link to={banner.path} className="group cursor-pointer block">
                  <div
                    className={`relative overflow-hidden rounded-2xl bg-gradient-to-br ${banner.bg} h-64 md:h-72`}
                  >
                    <img
                      src={banner.image}
                      alt={banner.title}
      ...
      <section className="py-24 bg-[#7A4E48] relative overflow-hidden">
        <div className="absolute inset-0 opacity-10">
          <div
            ...
            style={{
              backgroundImage:
                "url(\"data:image/svg+xml,%3Csvg width='60' height='60' viewBox='0 0 60 60' xmlns='http://www.w3.org/2000/svg'%3E%3Cg fill='none' fill-rule='evenodd'%3E%3Cg fill='%23C9A86A' fill-opacity='0.4'%3E%3Cpath d='M36 34v-4h-2v4h-4v2h4v4h2v-4h4v-2h-4zm0-30V0h-2v4h-4v2h4v4h2V6h4V4h-4zM6 34v-4H4v4H0v2h4v4h2v-4h4v-2H6zM6 4V0H4v4H0v2h4v4h2V6h4V4H6z'/%3E%3C/g%3E%3C/g%3E%3C/svg%3E\")",
            }}
          />
        </div>
        <div className="relative max-w-4xl mx-auto px-4 text-center">
          ...
          >
            ...
            <p className="font-body text-white/70 text-base md:text-lg mb-10 max-w-xl mx-auto leading-relaxed">
              From Diwali celebrations to wedding festivities, discover our exclusive festive
              collection crafted to make you shine.
            </p>
            <Link
              to="/collections"
              className="inline-flex items-center gap-2 bg-[#C9A86A] text-white font-body font-semibold text-sm px-10 py-4 rounded-full hover:bg-[#B08850] transition-colors cursor-pointer hover:shadow-lg hover:shadow-[#C9A86A]/30 hover:-translate-y-0.5"
            >
              Explore Festive Collection <FiArrowRight className="w-4 h-4" />
            </Link>
          </motion.div>
            ...
            {testimonials.map((t, i) => (
              <SwiperSlide key={t.name}>
                ...
                >
                  <div className="flex items-center gap-1 mb-4">
                    {Array.from({ length: t.rating }).map((_, j) => (
                      <span key={j} className="text-[#C9A86A] text-sm">
                        ★
                      </span>
                    ))}
                  </div>
                      ...
                      onError={(event) =>
                        setFallbackImage(
                          ...
                          `https://ui-avatars.com/api/?name=${encodeURIComponent(
                            t.name
                          )}&background=EFE7DC&color=7A4E48`
                        )
                      }
                      className="w-10 h-10 rounded-full object-cover"
                    />
