...
export default function ProductDetail() {
  ...
  const loadMyReview = async (targetProductId: number) => {
    if (!isAuthenticated) {
      setMyReview(null);
    ...
    try {
      const existing = await reviewsApi.getMyReview(targetProductId);
      setMyReview(existing);
      setReviewMode(existing ? null : 'create');
    } catch {
      setMyReview(null);
      setReviewMode('create');
    }
  ...
  const refreshProductAndReviews = async (targetProductId: number, silent = true) => {
    await Promise.all([loadReviews(targetProductId, silent), loadMyReview(targetProductId)]);

    try {
      const freshProduct = await productsApi.getById(targetProductId);
      setProduct(freshProduct);
    } catch {
      // Keep existing product details when a silent refresh fails.
    }
  };
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      if (document.visibilityState === 'visible') {
        refreshProductAndReviews(product.id, true);
      }
    };

    const interval = window.setInterval(refresh, REVIEW_REFRESH_MS);
    window.addEventListener('focus', refresh);
  ...
  const startEditReview = () => {
    if (!myReview) return;
    if (myReview.is_removed) {
      toast.error('This review was removed and cannot be edited.', { style: TOAST_STYLE });
      return;
    }
  ...
  const cancelEditReview = () => {
    setReviewRating(5);
    setReviewBody('');
    setReviewMode(myReview ? null : 'create');
  };
  ...
  const handleSubmitReview = async () => {
    ...
    if (cleanBody.length < 2) {
      toast.error('Review must be at least 2 characters.', { style: TOAST_STYLE });
      return;
    }

    if (myReview && reviewMode !== 'edit') {
      toast.error('You have already reviewed this product. Use Edit Review to update it.', { style: TOAST_STYLE });
      return;
    }

    setReviewSaving(true);
    try {
      const review = myReview && reviewMode === 'edit'
      ...
      toast.success(myReview && reviewMode === 'edit' ? 'Review updated successfully.' : 'Review added successfully.', {
        style: TOAST_STYLE,
      });
    } catch (err: any) {
      toast.error(err.response?.data?.error || 'Could not save review.', { style: TOAST_STYLE });
    } finally {
      setReviewSaving(false);
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          <div className="grid lg:grid-cols-2 gap-12">
            <div className="space-y-4">
              ...
              <div className="flex gap-3">
                {[1, 2, 3].map((item) => (
                  <div key={item} className="w-20 h-20 rounded-xl bg-[#EFE7DC] animate-pulse" />
                ))}
              </div>
            </div>
            <div className="space-y-4">
              {[1, 2, 3, 4, 5].map((item) => (
                <div key={item} className="h-5 bg-[#EFE7DC] rounded-full animate-pulse w-3/4" />
              ))}
            </div>
          </div>
        </div>
  ...
  const showReviewForm = isAuthenticated && (!myReview || reviewMode === 'edit');
  const reviewFormTitle = myReview && reviewMode === 'edit' ? 'Edit Your Review' : 'Write a Review';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
              ...
              <div className="flex gap-3 overflow-x-auto pb-1">
                {product.images.map((img, index) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-xl overflow-hidden flex-shrink-0 border-2 transition-all cursor-pointer ${
                      selectedImage === index ? 'border-[#7A4E48]' : 'border-transparent hover:border-[#C9A86A]'
                    }`}
                  >
                    <img
                      src={img}
            ...
            <div>
              ...
              <div className="flex items-center border border-[#E8DCCB] rounded-xl overflow-hidden w-fit bg-[#F8F5F0]">
                <button
                  onClick={() => setQuantity(Math.max(1, quantity - 1))}
                  disabled={quantity <= 1}
                  className="w-10 h-10 flex items-center justify-center hover:bg-[#EFE7DC] transition-colors cursor-pointer font-body text-lg text-[#555] disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  −
                </button>
                <span className="w-12 text-center font-body text-sm font-medium text-[#1F1F1F]">{quantity}</span>
                        ...
                        {!isAuthenticated ? (
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              >
                                {reviewSaving && <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />}
                                {reviewSaving ? 'Saving...' : myReview && reviewMode === 'edit' ? 'Update Review' : 'Submit Review'}
                              </button>
                              {myReview && reviewMode === 'edit' && (
                                <button
                                  onClick={cancelEditReview}
                                  disabled={reviewSaving}
                                  className="inline-flex items-center justify-center gap-2 border border-[#E8DCCB] bg-white text-[#555] font-body text-sm font-semibold px-5 py-3 rounded-xl hover:bg-[#EFE7DC] transition-colors cursor-pointer disabled:opacity-60"
                                >
                          ...
                          <div className="bg-white rounded-xl border border-red-100 px-4 py-3">
                            <p className="font-body text-sm font-semibold text-[#1F1F1F]">Your review was removed by admin.</p>
                            <p className="font-body text-xs text-[#999] mt-1">You cannot submit another review for this product.</p>
                          </div>
                        ) : (
                          <div className="bg-white rounded-xl border border-[#E8DCCB] px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                            <div>
                              <p className="font-body text-sm font-semibold text-[#1F1F1F]">You have already reviewed this product.</p>
                              <p className="font-body text-xs text-[#999] mt-1">
                                {myReview?.is_flagged
                                  ? 'Your review is currently hidden. Edit it and save again if you want to update it.'
                                  : 'You can edit your review instead of adding a second one.'}
                              </p>
                            </div>
                            <button
                              type="button"
                              onClick={startEditReview}
                              className="inline-flex items-center justify-center gap-2 bg-[#7A4E48] text-white font-body text-xs font-semibold px-4 py-2.5 rounded-xl hover:bg-[#5A3A36] transition-colors cursor-pointer"
                            >
                              <FiEdit2 className="w-3.5 h-3.5" /> Edit Review
                            </button>
                          </div>
                        )}
                      </div>
                                ...
                                <div>
                                  <p className="font-body text-sm font-semibold text-[#1F1F1F]">
                                    ...
                                    {user?.id === review.user_id && (
                                      <span className="ml-2 text-[10px] font-medium text-[#7A4E48] bg-[#EFE7DC] px-2 py-0.5 rounded-full">
                                        You
                                      </span>
                                    )}
                                  </p>
        ...
        {related.length > 0 && (
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 md:gap-6">
              {related.map((rp, index) => <ProductCard key={rp.id} product={rp as any} index={index} />)}
            </div>
          </section>
        )}
      </div>
