...
//                       reason === r ? 'text-[#1F1F1F] font-medium' : 'text-[#777]'
//                     }`}
//                   >
//                     {r}
//                   </span>
...
//             {silentRefreshing && (
//               <span className="font-body text-xs text-[#999]">Updating...</span>
//             )}
//           </div>
...
//                   {order.items.map((item) => (
//                     <div key={item.id} className="flex items-center gap-4">
//                       <div className="w-16 rounded-xl overflow-hidden bg-[#EFE7DC] flex-shrink-0" style={{ height: '72px' }}>
//                         <img
//                           src={item.product_image}
//                           alt={item.product_title}
//                           className="w-full h-full object-cover"
...
//                   <p className="font-body text-xs text-[#999] bg-[#F8F5F0] rounded-xl px-3 py-2.5">
              ...
              {CANCEL_REASONS.map((item) => (
                <label key={item} className="flex items-center gap-3 cursor-pointer group">
                  <button
                    ...
                    className={`w-4 h-4 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${
                      reason === item ? 'border-[#C58C85] bg-[#C58C85]' : 'border-[#CCC]'
                    }`}
                  >
                    {reason === item && <div className="w-1.5 h-1.5 rounded-full bg-white" />}
                  </button>

                  <span
                    onClick={() => setReason(item)}
                    className={`font-body text-sm cursor-pointer ${
                      reason === item ? 'text-[#1F1F1F] font-medium' : 'text-[#777]'
                    }`}
                  >
                    {item}
                  </span>
    ...
    <div className="pt-[72px] min-h-screen bg-[#F8F5F0]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        {loading ? (
          <div className="space-y-6">
            {[1, 2, 3].map((item) => (
              <div key={item} className="bg-white rounded-2xl border border-[#E8DCCB] p-6 animate-pulse h-32" />
            ))}
          </div>
        ) : error ? (
          <div className="text-center py-20">
          ...
          <div className="grid lg:grid-cols-3 gap-8">
                      ...
                      <div key={step} className="flex items-center flex-1">
                        <div className="flex flex-col items-center">
                          ...
                          <span
                            className={`font-body text-[10px] mt-1 capitalize text-center px-1.5 py-0.5 rounded-full ${
                              index === statusIndex
                                ? STATUS_COLORS[step] || 'text-[#7A4E48] font-semibold'
                                : index < statusIndex
                                  ? 'text-[#7A4E48] font-semibold'
                                  : 'text-[#CCC]'
                            }`}
                        ...
                        {index < STATUS_STEPS.length - 1 && (
                          <div
                            className={`flex-1 h-0.5 mx-2 mb-4 transition-colors ${
                              index < statusIndex ? 'bg-[#7A4E48]' : 'bg-[#E8DCCB]'
                            }`}
                          />
                        )}
                      </div>
            ...
            <div className="space-y-4">
              <div className="bg-white rounded-2xl border border-[#E8DCCB] p-6 shadow-sm sticky top-24">
                ...
                {!canCancel && !isCancelled && (
                  <p className="font-body text-xs text-[#999] bg-[#F8F5F0] rounded-xl px-3 py-2.5">
                    This order can no longer be cancelled because admin has already confirmed it.
                  </p>
                )}
