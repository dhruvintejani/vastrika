...
"""
app/api/v1/admin/reviews/routes.py
Reviews: customer submission/editing, public product reviews, and admin moderation.
"""
from datetime import datetime, timezone
from typing import Optional
...
from app.core.dependencies import CurrentAdminID, CurrentUserID, DBSession
from app.core.exceptions import BusinessRuleError, ForbiddenError, ConflictError, NotFoundError
from app.models.product import Product
from app.models.review import Review
from app.schemas.base import APIResponse, PaginatedResponse
from app.schemas.extras import ReviewCreateRequest, ReviewResponse, ReviewUpdateRequest
from app.utils.pagination import paginate
...
def _build_review_response(review: Review) -> ReviewResponse:
    return ReviewResponse(
        ...
        is_approved=review.is_approved,
        is_flagged=review.is_flagged,
        is_removed=(not review.is_approved and not review.is_flagged),
        removed_at=None,
        created_at=review.created_at,
    )
...
async def _load_user_product_review(
    db: AsyncSession,
    user_id: int,
    product_id: int,
) -> Optional[Review]:
...
async def _recalculate_rating(
    ...
    await db.execute(
        update(Product)
        .where(Product.id == product_id)
        .values(rating=round(float(avg_rating or 0), 2), review_count=count or 0)
    )
...
@router.get("/reviews/my", response_model=APIResponse[Optional[ReviewResponse]])
async def get_my_review(
    user_id: CurrentUserID,
    db: DBSession,
    product_id: int = Query(..., ge=1),
):
    review = await _load_user_product_review(db, user_id, product_id)
    return APIResponse(data=_build_review_response(review) if review else None)
...
async def submit_review(body: ReviewCreateRequest, user_id: CurrentUserID, db: DBSession):
    ...
    existing = await _load_user_product_review(db, user_id, body.product_id)
    if existing:
        if not existing.is_approved and not existing.is_flagged:
            raise ConflictError("Your previous review for this product was removed and cannot be submitted again")
        raise ConflictError("You have already reviewed this product. Please edit your existing review instead")
...
async def update_my_review(
    ...
    if review.user_id != user_id:
        raise ForbiddenError("You can edit only your own review")
    if not review.is_approved and not review.is_flagged:
        raise BusinessRuleError("This review was removed and cannot be edited")
    ...
    return APIResponse(data=_build_review_response(fresh_review), message="Review updated")
...
async def admin_list_reviews(
    ...
    if flagged is True:
        ...
        filters.append(Review.is_approved == False)  # noqa: E712
        filters.append(Review.is_flagged == False)  # noqa: E712
    else:
        filters.append(or_(Review.is_approved == True, Review.is_flagged == True))  # noqa: E712
