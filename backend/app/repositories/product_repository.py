...
class ProductRepository:
    ...
    async def list_products(
        ...
        if search:
            ...
            if clean_search:
                base_q = base_q.where(Product.title.ilike(f"%{clean_search}%"))
        ...
        base_q = (
            base_q
            .order_by(order_col, Product.created_at.desc(), Product.id.desc())
            .offset(skip)
            .limit(limit)
        )
    ...
    async def create(self, data: dict, variants: list[dict]) -> Product:
        ...
        await self.db.flush()
        await self.db.refresh(product)
        return await self.get_by_id(product.id, include_deleted=True)  # type: ignore

    async def update(self, product_id: int, data: dict) -> Optional[Product]:
        clean = {k: v for k, v in data.items() if v is not None}

        variants = clean.pop("variants", None)
        ...
        if variants is not None:
            ...
            for v in variants:
                if hasattr(v, "model_dump"):
                    v = v.model_dump()
                self.db.add(ProductVariant(product_id=product_id, **v))

            await self.db.flush()

        return await self.get_by_id(product_id, include_deleted=True)
...
class CategoryRepository:
    ...
    async def get_by_slug(self, slug: str, active_only: bool = False) -> Optional[Category]:
