...
import React from "react";
import { motion, AnimatePresence } from "framer-motion";
import { FiX, FiAlertTriangle, FiChevronDown, FiCheck, FiSearch } from "react-icons/fi";
import { OrderStatus } from "../../types";
        ...
        <AnimatePresence>
          {open && !disabled && (
            ...
            >
              <div className="p-1.5">
                {showSearch && (
                  <div className="sticky top-0 z-10 bg-admin-card p-1.5">
                    <div className="flex items-center gap-2 rounded-xl border border-admin-border bg-admin-bg px-3 py-2">
                      <FiSearch className="w-3.5 h-3.5 text-admin-muted" />
                      <input
                        value={query}
                        onChange={(e) => setQuery(e.target.value)}
                        placeholder="Search options..."
                        className="w-full bg-transparent font-body text-xs text-admin-text placeholder:text-admin-muted focus:outline-none"
                      />
                    </div>
                  </div>
                )}
                ...
                {filteredOptions.length === 0 ? (
                  <div className="px-3 py-2.5 font-body text-xs text-admin-muted">
                    No matching options
                  </div>
